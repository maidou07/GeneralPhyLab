...
BOUNDS = [
    (0.8, 1.0),        # R1
    (-np.pi/6, np.pi/6), # phi1
    (0.0, 5.0),       # gamma1
    (0.5, 0.8),        # R2
    (-np.pi/6, np.pi/6), # phi2
    (0.0, 40.0),       # gamma2
    (0, 0),        # R3
    (0.0, 0.0), # phi3
    (0, 0),       # gamma3
...
def objective(params, l_values_sim_np, l_peaks_exp_mm, amp_peaks_exp_norm, 
    ...
    # 5. Match peaks based on LOCATION ONLY and calculate optimal amplitude scale
    ...
    if num_sim_peaks_filt > 0:
        for i, l_exp in enumerate(l_peaks_exp_mm):
            ...
            nearest_sim_filt_idx = np.argmin(distances)
            # Match if distance is less than half the fundamental wavelength
            if distances[nearest_sim_filt_idx] < LAMBDA1_HALF_MM / 2.0:
                 # Allow potential many-to-one mapping for scaling calculation phase
                 matched_exp_to_sim[i] = nearest_sim_filt_idx
    logger.debug(f"[PID:{pid}] objective: Matched {len(matched_exp_to_sim)} peaks for scaling based on L distance < {LAMBDA1_HALF_MM / 2.0:.2f}mm.")
