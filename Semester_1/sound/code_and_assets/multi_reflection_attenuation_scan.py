# sound/py/multi_reflection_attenuation_scan.py
...
def calculate_pressure_at_points_pytorch(field_points_pt, source_positions_pt, source_amplitudes_pt, k_pt, gamma_pt):
    ...
    # gamma_pt: scalar attenuation coefficient (Np/m)
    ...
    # Calculate contributions including attenuation and spherical spreading
    # Broadcasting: (1, Ns) * (Nf, Ns) * (Nf, Ns) / (Nf, Ns) -> (Nf, Ns)
...
# === 定义要测试的反射系数值、相位和衰减系数 ===
R_val = 0.95  # Fixed R
phi_R_val = 0.0 # Fixed phi_R
# Updated gamma values based on typical air attenuation at ~37kHz
# gamma_values = np.array([0.1, 0.2, 0.3, 0.4], dtype=np.float32) # Physically realistic but small effect
# Using larger values to visualize the attenuation effect clearly
gamma_values = np.array([0, 5, 10, 20], dtype=np.float32) # Attenuation coefficient (Np/m)

print(f"固定反射系数 R = {R_val}")
print(f"固定反射相位 phi_R = {phi_R_val} rad")
print(f"将测试衰减系数 gamma (Np/m): {gamma_values}")
...
for gamma_val in gamma_values:
    gamma_pt = torch.tensor(gamma_val, dtype=torch.float32, device=device)
    print(f"\n===== 计算衰减 gamma = {gamma_val:.2f} Np/m =====")

    P_total_multi_at_l_np = np.zeros(num_l_points, dtype=np.complex64)

    print(f"  循环计算 l 处声压 (N={N_pairs}, R={R_val:.2f}, phi={phi_R_val:.2f}, gamma={gamma_val:.2f}) 使用 PyTorch..." )
    ...
    for i, l_val_np in enumerate(l_values_np):
        ...
        # Call PyTorch function including gamma
        ...
        if (i + 1) % max(1, num_l_points // 10) == 0:
             ...
             print(f"    l-scan (gamma={gamma_val:.2f}) 进度: {percent_done:.1f}% ({processed_points}/{num_l_points}), Speed: {points_per_sec:.1f} l/s, ETA: {eta:.1f}s")

    print(f"  l-scan (gamma={gamma_val:.2f}) 计算完成，耗时 {time.time() - start_scan_time:.2f} 秒。")

    # --- 分析当前 gamma 值的振幅和 SPL ---
    P_amplitude = np.abs(P_total_multi_at_l_np)
    # Use a fixed reference pressure (e.g., max amplitude from gamma=0 case if calculated, or simply max of current curve)
    ...
    if p_ref <= 0 or not np.isfinite(p_ref):
         print(f"警告: gamma={gamma_val:.2f} 时计算得到无效的最大声压 {p_ref}，跳过 SPL 计算。")
    ...
    # --- 绘制当前 gamma 值的独立图 ---
    print(f"  绘制 gamma={gamma_val:.2f} 的独立图表...")
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)
    # Update title to include gamma
    fig.suptitle(f'中心点声压 vs l (R={R_val:.2f}, $\phi_R$={phi_R_val:.2f}$\pi$, $\gamma$={gamma_val:.1f} Np/m, N={N_pairs})', fontsize=14)
    ...
    # Update filename to include gamma
    gamma_suffix = f"{gamma_val:.1f}".replace('.', 'p')
    save_path = os.path.join(img_dir, f'multi_reflection_attenuation_scan_R{R_val:.2f}_phi{phi_R_val:.2f}_gamma{gamma_suffix}_N{N_pairs}.pdf')
    try:
