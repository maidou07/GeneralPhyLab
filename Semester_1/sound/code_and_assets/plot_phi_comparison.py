...
import logging
import argparse
from scipy.signal import find_peaks # Keep for potential future use, though not used in this version
...
def setup_logger(log_filename='phi_comparison.log'):
    logger = logging.getLogger('phi_comparison')
...
def calculate_pressure_at_points_pytorch(field_points_pt, source_positions_pt, source_amplitudes_pt, k_pt, gamma_pt):
    # (Same as in plot_R_comparison.py)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Plot pressure amplitude vs distance for different phi values (R=0.95 fixed).')
    # Harmonic control arguments
    ...
    # Output arguments
    parser.add_argument('--output', type=str, default='sound/img/phi_comparison_plot', help='Base output plot file path (without extension).')
    parser.add_argument('--log_file', type=str, default='sound/logs/phi_comparison.log', help='Log file path.')
    # Simulation range arguments
    ...
    logger.info("--- Starting Phi Comparison Plot Generation (R=0.95) ---")
    ...
    # --- Define Fixed R and Parameters based on harmonic flags ---
    R_fixed = 0.95
    logger.info(f"Fixed R = {R_fixed:.2f}")
    if args.disable_higher_harmonics:
        # Parameters: gam1 (phi will be inserted)
        fixed_params_tuple = (1.0,)
        k_vals_np = (K1_NP,)
        param_indices = {"gam1": 2} # Indices in the 3-param list [R1, phi1, gam1]
        num_sim_params = 3
        phi_param_indices = [1] # Index of phi1
        title_suffix = "(1st Harmonic Only)"
        logger.info("Simulating only 1st harmonic.")
    elif args.disable_third_harmonic:
        # Parameters: gam1, gam2, A2 (phi1, phi2 will be inserted)
        fixed_params_tuple = (1.0, 5.0, 0.3)
        k_vals_np = (K1_NP, K2_NP)
        param_indices = {"gam1": 2, "gam2": 5, "A2": 6} # Indices in the 7-param list
        num_sim_params = 7
        phi_param_indices = [1, 4] # Indices of phi1, phi2
        title_suffix = "(1st & 2nd Harmonics)"
        logger.info("Simulating 1st and 2nd harmonics.")
    else:
        # Parameters: gam1, gam2, A2, gam3, A3 (phi1, phi2, phi3 will be inserted)
        fixed_params_tuple = (1.0, 5.0, 0.3, 10.0, 0.1)
        k_vals_np = (K1_NP, K2_NP, K3_NP)
        param_indices = {"gam1": 2, "gam2": 5, "A2": 9, "gam3": 8, "A3": 10} # Indices in the 11-param list
        num_sim_params = 11
        phi_param_indices = [1, 4, 7] # Indices of phi1, phi2, phi3
    ...
    logger.info(f"Fixed parameters (excluding R and phi): {fixed_params_tuple}")
    ...
    # --- Simulation Loop for Phi --- 
    # Define phi values to plot (in radians)
    phi_values_to_plot_rad = np.linspace(-np.pi/6, np.pi/6, 5) # Example: 5 steps from -pi/6 to pi/6
    results = {} # Store P_amplitude for each phi

    logger.info(f"Starting simulations for phi (radians) = {phi_values_to_plot_rad}...")
    sim_start_time = time.time()
    for phi_rad in phi_values_to_plot_rad:
        iter_start_time = time.time()
        logger.info(f"  Simulating for phi = {phi_rad:.3f} rad ({np.degrees(phi_rad):.1f} deg)...")

        # Construct the parameter list for simulate_pressure_curve
        current_params = [0.0] * num_sim_params
        current_params[0] = R_fixed # R1
        current_params[param_indices["gam1"]] = fixed_params_tuple[0]

        # Set phi for all active harmonics
        for idx in phi_param_indices:
             if idx < num_sim_params: # Check if index is valid for current harmonic setting
                 current_params[idx] = phi_rad
        
        if num_sim_params >= 7: # If 2nd harmonic is included
            current_params[3] = R_fixed # R2
            current_params[param_indices["gam2"]] = fixed_params_tuple[1]
            current_params[param_indices["A2"]] = fixed_params_tuple[2]
            
        if num_sim_params >= 11: # If 3rd harmonic is included
            current_params[6] = R_fixed # R3
            current_params[param_indices["gam3"]] = fixed_params_tuple[3]
            current_params[param_indices["A3"]] = fixed_params_tuple[4]
        ...
        results[phi_rad] = P_amp
        iter_end_time = time.time()
        logger.info(f"    -> phi = {phi_rad:.3f} simulation finished in {iter_end_time - iter_start_time:.2f} s")
    ...
    try:
        ...
        colors = plt.cm.coolwarm(np.linspace(0, 1, len(phi_values_to_plot_rad))) # Use a diverging colormap for phase
        max_overall_amp = 0

        for i, phi_rad in enumerate(phi_values_to_plot_rad):
            P_amp = results.get(phi_rad)
            if P_amp is not None and len(P_amp) > 0:
                current_max = np.max(P_amp)
                if current_max > max_overall_amp: max_overall_amp = current_max
                ax.plot(l_values_sim_mm, P_amp, label=f'$\phi$ = {np.degrees(phi_rad):.1f}\degree', color=colors[i], linewidth=1.5, alpha=0.8)
            else: logger.warning(f"No simulation data for phi = {phi_rad:.3f}, skipping main plot.")

        ax.set_xlabel('Distance from Source (mm)', fontsize=13)
        ax.set_ylabel('Simulated Pressure Amplitude (Arb. Units)', fontsize=13)
        ax.set_title(f'Effect of Phase ($\phi$) on Pressure Amplitude (R={R_fixed:.2f}) {title_suffix}', fontsize=15, pad=15)
        ax.legend(title="Phase $\phi$", fontsize=11, title_fontsize=12, loc='upper right')
        ax.grid(True, which='major', linestyle='--', alpha=0.7)
        ax.grid(True, which='minor', linestyle=':', alpha=0.5)
        ax.tick_params(axis='both', which='major', labelsize=11)
        ax.minorticks_on()
        ax.set_xlim(args.L_min * 1000, args.L_max * 1000)
        if max_overall_amp > 0: ax.set_ylim(bottom=0, top=max_overall_amp * 1.1)

        # --- Inset Plot 1: Near Field (dB) --- 
        axins1 = ax.inset_axes([0.05, 0.55, 0.35, 0.35])
        ...
        for phi_rad in phi_values_to_plot_rad:
            ...
            if P_amp is not None and len(P_amp) > 0:
                ...
                if np.any(mask): max_near_amp = max(max_near_amp, np.max(P_amp[mask]))
        ...
        for i, phi_rad in enumerate(phi_values_to_plot_rad):
            ...
            if P_amp is not None and len(P_amp) > 0 and max_near_amp > 0:
                ...
                mask = (l_values_sim_mm >= x1_near) & (l_values_sim_mm <= x2_near)
                if np.any(mask): min_db_near = min(min_db_near, np.min(P_db_near[mask]))
        lower_db_limit_near = max(min_db_near - 5, -60)
        axins1.set_ylim(lower_db_limit_near, 3)
        ...
        ax.indicate_inset_zoom(axins1, edgecolor="black", alpha=0.8)

        # --- Inset Plot 2: Far Field (Linear Amplitude) ---
        axins2 = ax.inset_axes([0.55, 0.55, 0.4, 0.35])
        ...
        for i, phi_rad in enumerate(phi_values_to_plot_rad):
            ...
            if P_amp is not None and len(P_amp) > 0:
                ...
                if np.any(mask): max_far_amp = max(max_far_amp, np.max(P_amp[mask]))
        if max_far_amp > 0: axins2.set_ylim(0, max_far_amp * 1.1)
        else: axins2.set_ylim(0, 1)
        axins2.set_title(f'Far Field Detail ({x1_far:.1f}-{x2_far:.1f} mm)', fontsize=10)
        axins2.set_ylabel("Amplitude (Arb. Units)", fontsize=10)
        axins2.tick_params(axis='both', labelsize=9)
        axins2.grid(True, alpha=0.5, linestyle=':')
        ax.indicate_inset_zoom(axins2, edgecolor="black", alpha=0.8)
        ...
        except Exception as e: logger.error(f"Failed to save main PNG plot to {main_png_path}: {e}")
        try:
            plt.savefig(main_pdf_path, format='pdf')
            logger.info(f"Main plot saved successfully to: {main_pdf_path}")
        except Exception as e: logger.error(f"Failed to save main PDF plot to {main_pdf_path}: {e}")

        # --- Create and Save Separate Near-Field Plot (dB) ---
        logger.info("Generating separate near-field plot...")
        try:
            fig_near, ax_near = plt.subplots(figsize=(8, 5))
            max_near_amp_standalone = max_near_amp # Use already calculated max
            near_epsilon_standalone = near_epsilon
            min_db_near_standalone = 0
            for i, phi_rad in enumerate(phi_values_to_plot_rad):
                 P_amp = results.get(phi_rad)
                 if P_amp is not None and len(P_amp) > 0 and max_near_amp_standalone > 0:
                     P_amp_safe = np.maximum(P_amp, near_epsilon_standalone)
                     P_db_near = 20 * np.log10(P_amp_safe / max_near_amp_standalone)
                     ax_near.plot(l_values_sim_mm, P_db_near, color=colors[i], linewidth=1.5, alpha=0.9, label=f'$\phi$ = {np.degrees(phi_rad):.1f}\degree')
                     mask = (l_values_sim_mm >= x1_near) & (l_values_sim_mm <= x2_near)
                     if np.any(mask): min_db_near_standalone = min(min_db_near_standalone, np.min(P_db_near[mask]))
                 else: pass
            ax_near.set_xlim(x1_near, x2_near)
            lower_db_limit_near_standalone = max(min_db_near_standalone - 5, -60)
            ax_near.set_ylim(lower_db_limit_near_standalone, 3)
            ax_near.set_xlabel('Distance from Source (mm)', fontsize=12)
            ax_near.set_ylabel('Amplitude (dB re max in range)', fontsize=12)
            ax_near.set_title(f'Near Field Zoom ({x1_near:.0f}-{x2_near:.0f} mm) - R={R_fixed:.2f} {title_suffix}', fontsize=14)
            ax_near.legend(title="Phase $\phi$", fontsize=10, title_fontsize=11)
            ax_near.grid(True, which='major', linestyle='--', alpha=0.7)
            ax_near.grid(True, which='minor', linestyle=':', alpha=0.5)
            ax_near.tick_params(axis='both', labelsize=10)
            ax_near.minorticks_on()
            plt.tight_layout()
            near_png_path = base_output_path + "_near_field_zoom.png"
            near_pdf_path = base_output_path + "_near_field_zoom.pdf"
            try:
                plt.savefig(near_png_path, dpi=150)
                logger.info(f"Separate near-field plot saved to: {near_png_path}")
            except Exception as e: logger.error(f"Failed to save separate PNG plot to {near_png_path}: {e}")
            try:
                plt.savefig(near_pdf_path, format='pdf')
                logger.info(f"Separate near-field plot saved to: {near_pdf_path}")
            except Exception as e: logger.error(f"Failed to save separate PDF plot to {near_pdf_path}: {e}")
        except Exception as near_plot_err: logger.error(f"An error occurred during separate near-field plotting: {near_plot_err}")
        finally: 
             if 'fig_near' in locals() and plt.fignum_exists(fig_near.number): plt.close(fig_near)

        # --- Create and Save Separate Far-Field Plot (Linear) ---
        logger.info("Generating separate far-field plot...")
        try:
            fig_far, ax_far = plt.subplots(figsize=(8, 5))
            max_far_amp_standalone = max_far_amp # Use already calculated max
            for i, phi_rad in enumerate(phi_values_to_plot_rad):
                 P_amp = results.get(phi_rad)
                 if P_amp is not None and len(P_amp) > 0:
                     ax_far.plot(l_values_sim_mm, P_amp, color=colors[i], linewidth=1.5, alpha=0.9, label=f'$\phi$ = {np.degrees(phi_rad):.1f}\degree')
                 else: pass
            ax_far.set_xlim(x1_far, x2_far)
            if max_far_amp_standalone > 0: ax_far.set_ylim(0, max_far_amp_standalone * 1.1)
            else: ax_far.set_ylim(0, 1)
            ...
            ax_far.set_title(f'Far Field Zoom ({x1_far:.1f}-{x2_far:.1f} mm) - R={R_fixed:.2f} {title_suffix}', fontsize=14)
            ax_far.legend(title="Phase $\phi$", fontsize=10, title_fontsize=11)
            ax_far.grid(True, which='major', linestyle='--', alpha=0.7)
            ax_far.grid(True, which='minor', linestyle=':', alpha=0.5)
            ax_far.tick_params(axis='both', labelsize=10)
            ...
            except Exception as e: logger.error(f"Failed to save separate PNG plot to {far_png_path}: {e}")
            try:
                plt.savefig(far_pdf_path, format='pdf')
                logger.info(f"Separate far-field plot saved to: {far_pdf_path}")
            except Exception as e: logger.error(f"Failed to save separate PDF plot to {far_pdf_path}: {e}")
        except Exception as far_plot_err: logger.error(f"An error occurred during separate far-field plotting: {far_plot_err}")
        finally: 
             if 'fig_far' in locals() and plt.fignum_exists(fig_far.number): plt.close(fig_far)
